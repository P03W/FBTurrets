...
public class FBTurretsClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ClientSidePacketRegistry.INSTANCE.register(FBTurrets.TURRET_GUN_UPDATE,
            (packetContext, attachedData) -> {
                BlockPos pos = attachedData.readBlockPos();
                String gunID = attachedData.readString();
                packetContext.getTaskQueue().execute(() -> {
                        World world = packetContext.getPlayer().world;
                        BlockEntity blockEntity = world.getBlockEntity(pos);
                        if (blockEntity instanceof TurretHolderBlockEntity) {
                            ((TurretHolderBlockEntity)blockEntity).gun = (TurretGun)Registry.BLOCK.get(new Identifier(gunID));
                        }
                    }
                );
            }
